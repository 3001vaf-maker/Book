...
DEMO / First Run является таким же потребителем Shared UI. Для уже мигрированных рабочих разделов он не имеет права подменять реальный H/F/E/Z собственной `focusedShell`-оболочкой: подсказки и прогресс DEMO должны работать поверх тех же экранов и тех же F/E/Z, которые затем используются в обычной работе.
