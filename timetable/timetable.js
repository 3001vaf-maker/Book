...
export function renderTimetable(root) {
  ...
  root.innerHTML = `<div class="timetable-view">${pageHeader('График')}<div data-timetable-calendar></div><div class="cost-range paired-fields">${timeInput({ label: 'Начало', name: 'startTime', value: startTime })}${timeInput({ label: 'Окончание', name: 'endTime', value: endTime })}</div><div class="profile-actions"><button type="button" class="ui-button" data-timetable-apply disabled>Применить: рабочий день</button></div></div>`;
