...
async function persistReconciled(value) {
  const documentsResponse = await apiRequest('/document-archive/documents', {
    method: 'PUT',
    body: JSON.stringify({ value: value.documents }),
  });
  await responseJson(documentsResponse, 'Не удалось обновить документы');

  const historyResponse = await apiRequest('/document-archive/history', {
    method: 'PUT',
...
export async function initializeDocumentArchive() {
  configureBookDocumentBases(getBookDocumentBases(), {
    profile: getProfile(),
    workplaces: getWorkplaces(),
  });

  const remoteResponse = await apiRequest('/document-archive');
  ...
  const bootstrapResponse = await apiRequest('/document-archive/bootstrap', {
    method: 'POST',
