...
export class InvitationService {
  ...
  async resendInvitation(adminId: string, invitationId: string) {
    ...
    if (invitation.status !== InvitationStatus.PENDING) throw new ConflictException('Это приглашение уже не активно');
  ...
  private async findActiveInvitation(token: string) {
    ...
    if (invitation.status !== InvitationStatus.PENDING) throw new ConflictException('Приглашение уже использовано или отозвано');
