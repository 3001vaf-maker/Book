...
export class OnlineBookingService {
  ...
  private async accountView(tenantId: string, account: any) {
    ...
    const cardStats = await this.personCards.cardStats(tenantId, account);
  ...
  async registerAccount(tenantId: string, body: Record<string, any>) {
    ...
    const card = await this.personCards.cardState(tenantId, account.phone);
    const binding = card.owner
      ? await this.personCards.bindFirstAccess(tenantId, account as any)
      : {
  ...
  async loginAccount(tenantId: string, email: unknown, password: unknown) {
    ...
    const binding = await this.personCards.bindFirstAccess(tenantId, account as any);
  ...
  async updateAccount(tenantId: string, accountId: string, body: Record<string, any>) {
    ...
    await this.personCards.bindFirstAccess(tenantId, updated as any);
  ...
  async getMyRequests(tenantId: string, accountId: string) {
    ...
    const manualViews = await this.personCards.manualRecordViews(tenantId, account as any, importedRecordIds);
