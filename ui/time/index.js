...
export function wheel({values,selected,type,formatter=(v)=>String(v).padStart(2,'0')}={}){
  return values.map(value=>`<button type="button" class="time-wheel__item${String(value)===String(selected)?' is-selected':''}" data-time-wheel-item data-time-wheel-type="${esc(type)}" data-value="${esc(value)}">${esc(formatter(value))}</button>`).join('');
}
