...
export function workLinks({ links = [], name = 'links' } = {}) {
  ...
  return `<div class="work-links" data-work-links="${escape(name)}">${rows.map(linkRow).join('')}</div>`;
}

function linkRow(link = {}) {
  return `<div class="array-row link-row" data-work-link-row>${select({ name: 'linkType', value: link.type || 'Instagram', options: WORK_LINK_TYPES })}<input name="linkUrl" value="${escape(link.url || '')}" placeholder="URL"><button type="button" class="remove-button" data-remove-work-link aria-label="Удалить ссылку">×</button></div>`;
}
