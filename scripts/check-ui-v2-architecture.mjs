...
expect(css.includes('.v2-layer-backdrop--contained{position:fixed;inset:0') && css.includes('.v2-layer-backdrop--technical{position:fixed;inset:0') && css.includes('.v2-z:not(.v2-z--layer).has-v2-layer{transform:translateZ(0);overflow:hidden}') && ui.includes("host.classList.add('has-v2-layer')"), 'Shared work modals must be contained by the active Z surface while rare technical overlays may cover the whole system.');
...
expect(css.includes('.v2-app.has-e-deck .v2-deck{width:calc(var(--v2-fe-open-x) - var(--v2-gap))}') && css.includes('.v2-app.has-e-deck .v2-e-deck{width:calc(var(--v2-fe-open-x) - var(--v2-gap))}'), 'An F that owns E may use the 40% FE width, but E must share the same F footprint instead of sitting beside it.');
expect(/\.v2-deck__card\{[\s\S]*?bottom:0;[\s\S]*?height:auto;[\s\S]*?pointer-events:none;/.test(css) && /\.v2-deck__card strong\{[\s\S]*?pointer-events:auto;[\s\S]*?touch-action:none/.test(css), 'Every F must remain a full card while only its exposed 48px handle owns the grab gesture.');
expect(/\.v2-e-deck\{[\s\S]*?z-index:11;[\s\S]*?bottom:0;[\s\S]*?height:var\(--v2-e-height,var\(--v2-e-max-height\)\);[\s\S]*?max-height:var\(--v2-e-max-height\)/.test(css) && css.includes('--v2-e-max-height:66.667%'), 'E must be layered above F, bottom-anchored, and capped at the lower two thirds.');
expect(/\.v2-e-card\{[\s\S]*?pointer-events:none;/.test(css) && /\.v2-e-card strong\{[\s\S]*?pointer-events:auto;[\s\S]*?touch-action:none/.test(css), 'Only visible E handles may own pointer input; E bodies must never block another visible handle.');
...
expect(ui.includes('const syncEGeometry = () =>') && ui.includes('activeHeight * (2 / 3)') && ui.includes("eDeck.style.setProperty('--v2-e-height'"), 'E height must be derived from the real active F height, not from viewport approximation.');
