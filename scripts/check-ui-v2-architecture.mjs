...
expect(css.includes('--v2-card-handle:48px') && css.includes('--v2-card-handle-min:44px') && /\.v2-deck__card strong\{[\s\S]*?height:var\(--v2-card-handle\);[\s\S]*?transform:none;[\s\S]*?touch-action:none/.test(css), 'F handle must be horizontal, 48px by default, never below 44px, and must own the touch gesture.');
expect(/\.v2-deck__card\{[\s\S]*?top:var\(--v2-depth-y,0px\);[\s\S]*?bottom:0;[\s\S]*?height:auto;[\s\S]*?border-radius:0 var\(--v2-z-radius\) 0 0/.test(css), 'Every F layer must remain a full card to the bottom; the 48px value is only the exposed handle band.');
expect(css.includes('.v2-app.has-e-deck .v2-deck{width:calc(var(--v2-fe-open-x) - var(--v2-gap))}') && css.includes('.v2-app.has-e-deck .v2-e-deck{width:calc(var(--v2-fe-open-x) - var(--v2-gap))}'), 'When F owns E, the hierarchy may occupy the 40% FE width, but E must share the same F footprint instead of sitting beside it.');
expect(/\.v2-e-deck\{[\s\S]*?z-index:11;[\s\S]*?bottom:0;[\s\S]*?height:var\(--v2-e-height,66\.667%\);[\s\S]*?max-height:66\.667%/.test(css), 'E must sit above active F, anchored to its bottom, and never occupy more than the lower two thirds of F.');
expect(/\.v2-e-card strong\{[\s\S]*?min-width:var\(--v2-card-handle-min\);[\s\S]*?height:var\(--v2-card-handle\);[\s\S]*?transform:none;[\s\S]*?touch-action:none/.test(css), 'E handle must stay directly reachable with the same 48px/44px physical grab target.');
...
expect(ui.includes('const syncEGeometry = () =>') && ui.includes('activeHeight * (2 / 3)') && ui.includes("eDeck.style.setProperty('--v2-e-height'"), 'E height must be derived from the real active F height, not from viewport guesswork.');
