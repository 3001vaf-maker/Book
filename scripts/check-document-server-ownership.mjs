...
if (!core.includes('initializeTenantDocumentArchive')) failures.push('Core must initialize server-owned Documents before workspace render.');
if (!migration.includes("apiRequest('/tenant-document-archive')") || !migration.includes("apiRequest('/tenant-document-archive/bootstrap'")) failures.push('Documents startup must read/bootstrap the server owner directly.');
if (/localStorage|readLegacy|\/tenant-document-archive\/migrate/.test(migration)) failures.push('Documents startup must not depend on browser migration paths.');
...
if (!moduleSource.includes("import { AuthModule } from '../auth/auth.module';") || !moduleSource.includes('imports: [AuthModule, BusinessStateModule]')) failures.push('TenantDocumentArchiveModule must import AuthModule for JwtAuthGuard/JwtService runtime wiring.');
