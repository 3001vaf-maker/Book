...
| `initViewNavigation()` | Подключает поведение переключателя представлений | `ui/view-navigation/` | Инфраструктура UI | Журнал и другие разделы с несколькими представлениями | DOM View Navigation | ОБЩАЯ |
...
| `list()` | Универсальный простой перечень запрошенных значений; заголовок является основным, все остальные запрошенные данные — второстепенными | `ui/lists/` | Общая | Core UI; Журнал, График и любые другие экраны, которым нужен перечень | Любые данные, переданные вызывающей стороной; сущности компонент не знает | КАНОНИЧЕСКАЯ |
...
`stateView()` — композиция состояния из нескольких самостоятельных блоков. Каждый блок визуально отделён от соседних и может вести к своей корректировке. `StateView` использует визуальный принцип `list()` внутри блоков, но не является перечнем сущностей.
