...
export function renderJournalDay(root, { date = new Date(), workplaceId = '', onChange = () => {} } = {}) {
  ...
  contentRoot.innerHTML = journalDayTimeline({ from: time.from, to: time.to, records: getRecordsForDay(dateKey(date), workplaceId) });
  initJournalDayTimeline(contentRoot, { onSlotClick: ({ from, to }) => openRecordCreation({ date, workplaceId, from, to, onCreated: () => renderJournalDay(root, { date, workplaceId, onChange }) }) });
}
