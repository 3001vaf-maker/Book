...
function usagesForDay(date, workplaceId, records = readList(KEY), breaks = readList(BREAKS_KEY)) {
  ...
  return [
    ...records.filter((item) => item?.date === day && normalizeId(item?.workplaceId) === workplace && item?.status !== 'cancelled')
      .map((item) => ({ ...item, type: 'record', sourceId: item.id })),
    ...breaks.filter((item) => item?.date === day && normalizeId(item?.workplaceId) === workplace)
...
export function createRecord({ date, workplaceId, from, to, client, procedures = [] } = {}) {
  ...
  const check = checkRecordTime({ date: normalizedDate, workplaceId: normalizedWorkplaceId, from, to });
...
export function updateRecord(id, patch = {}) {
  ...
  if (next.status !== 'cancelled') {
    const check = checkRecordTime({ date: next.date, workplaceId: next.workplaceId, from: next.from, to: next.to, excludeId: id });
    if (!check.ok) return null;
  }
...
export function moveRecord(id, { date, workplaceId, from, to } = {}) {
  return updateRecord(id, { date: normalizeDate(date), workplaceId: normalizeId(workplaceId), from: String(from || ''), to: String(to || '') });
}
