...
function getBookingStep() {
  try {
    const profile = JSON.parse(localStorage.getItem('book.profile') || '{}');
    const value = Number(profile?.bookingStep);
...
function openTimePicker(state, original, onDone) {
  ...
  const duration = currentFrom != null && currentTo != null && currentTo > currentFrom ? currentTo - currentFrom : 60;
  const step = getBookingStep();
  ...
  if (start != null && end != null) {
    for (let minutes = start; minutes + duration <= end; minutes += step) {
      const from = minutesToTime(minutes);
      const to = minutesToTime(minutes + duration);
      const check = checkRecordTime({ date: state.date, workplaceId: state.workplaceId, from, to, excludeId: original.id });
