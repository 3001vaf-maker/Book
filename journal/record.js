import { button, durationText, escapeHtml, iconButton, list, listEntry, stateView, initStateView, initCalendar, mountModal, modal, timePicker, initTimePickers, initMultiSelect, viewNavigation, initViewNavigation } from '../ui/ui.js?v=journal-architecture-20260908';
...
function openTimeModal({ date, workplaceId, from, to, onCreated }) {
  ...
  initViewNavigation(m, { views: RECORD_MODES, activeView: 'record', onChange: (nextMode) => { if (nextMode !== 'block') return; m.remove(); openBlockEndModal({ date, workplaceId, from, onCreated }); } });
  bindTimes();
}
...
function openProceduresModal({ date, workplaceId, from, to, onCreated }) {
  ...
  const content = `<div class="record-screen record-screen--procedures"><div class="record-modal-toolbar"><strong>Процедуры</strong>${iconButton('+', { className: 'icon-button--primary', data: 'data-record-add', aria: 'Добавить процедуру' })}</div><div data-record-procedures></div><div class="record-modal-actions modal-actions" data-record-actions></div></div>`;
...
function openBlockEndModal({ date, workplaceId, from, onCreated }) {
  ...
  const m = mountModal(document.body, modal(html, { className: 'record-modal record-modal--full' })); if (!m) return; m.querySelectorAll('[data-block-end]').forEach((node) => node.addEventListener('click', () => { const to = node.dataset.blockEnd; if (!createJournalBreak({ workplaceId: String(workplaceId || ''), date: dateKey(date), from: String(from), to: String(to) })) return; m.remove(); onCreated?.(); }));
}
