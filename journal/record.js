import { button, durationText, entityCard, escapeHtml, list, listEntry, stateView, initStateView, initCalendar, mountModal, modal, timePicker, initTimePickers, initMultiSelect } from '../ui/ui.js?v=single-button-20260908';
...
function scopedUsages(date, workplaceId) {
  ...
  return getTimeUsages({
    records: readList('book.records').filter((item) => item?.date === day && item?.workplaceId === String(workplaceId || '')),
    breaks: readList('book.journalBreaks').filter((item) => item?.date === day && item?.workplaceId === String(workplaceId || '')),
  });
}
function nextFiveMinutes(from) { const start = timeToMinutes(from); if (start == null) return []; const hourStart = Math.floor(start / 60) * 60, hourEnd = hourStart + 60, result = []; for (let value = Math.ceil(start / 5) * 5; value < hourEnd; value += 5) result.push(minutesToTime(value)); return result; }
function openTimeModal({ date, workplaceId, from, to, onCreated }) {
  const usages = scopedUsages(date, workplaceId);
  const values = nextFiveMinutes(from).filter((value) => isTimeRangeAvailable({ from: value, to: minutesToTime(timeToMinutes(value) + 5), usages }));
  ...
  const m = mountModal(document.body, modal(`<div class="record-screen record-screen--time">${toggle}<div class="record-time-list">${times || '<div class="muted">Нет свободного времени</div>'}</div></div>`, { className: 'record-modal record-modal--full' })); if (!m) return;
  const bindTimes = () => m.querySelectorAll('[data-record-time]').forEach((node) => node.addEventListener('click', () => { const selected = node.dataset.recordTime; m.remove(); openProceduresModal({ date, workplaceId, from: selected, to, onCreated }); }));
  m.querySelectorAll('[data-record-mode-value]').forEach((node) => node.addEventListener('click', () => { m.querySelectorAll('[data-record-mode-value]').forEach((item) => item.classList.toggle('is-active', item === node)); if (node.dataset.recordModeValue === 'block') { m.remove(); openBlockEndModal({ date, workplaceId, from, onCreated }); return; } m.querySelector('.record-time-list').innerHTML = times || '<div class="muted">Нет свободного времени</div>'; bindTimes(); })); bindTimes();
}
...
function openProceduresModal({ date, workplaceId, from, to, onCreated }) {
  ...
  const syncActions = () => { const hasSelection = selected.size > 0; const actions = m.querySelector('[data-record-actions]'); if (!actions) return; actions.innerHTML = hasSelection ? `${button('Сброс выбора', { data: 'data-record-reset', variant: 'secondary' })}${button('Далее →', { data: 'data-record-next' })}` : ''; if (hasSelection) actions.querySelector('[data-record-reset]')?.addEventListener('click', () => { selected.clear(); selectionController?.setSelectedValues([]); }); if (hasSelection) actions.querySelector('[data-record-next]')?.addEventListener('click', () => { const duration = [...selected.values()].reduce((sum, item) => sum + (Number(item.duration) || 0), 0), end = minutesToTime(timeToMinutes(from) + duration), usages = scopedUsages(date, workplaceId); if (!isTimeRangeAvailable({ from, to: end, usages })) { alert('Это время уже занято.'); return; } m.remove(); openClientModal({ date, workplaceId, from, to: end, procedures: [...selected.values()], onCreated }); }); };
  const render = () => { const host = m.querySelector('[data-record-procedures]'); host.innerHTML = list({ items: items.map((procedure) => { const cost = defaultCost(procedure, workplaceId); const secondary = [durationText(procedure.duration), cost !== '' ? `${cost} ₽` : '']; return { title: procedure.name || '', secondary, interactive: true, data: `data-procedure-select="${escapeHtml(procedure.id)}"`, aria: `Выбрать процедуру ${procedure.name || ''}`, selected: selected.has(procedure.id) }; }) }) || '<div class="muted">Процедур для этого места работы пока нет.</div>'; syncActions(); selectionController?.destroy(); selectionController = initMultiSelect(host, { selectedValues: [...selected.keys()], selector: '[data-procedure-select]', valueAttribute: 'procedureSelect', onChange: (values) => { const next = new Map(); values.forEach((id) => { const procedure = items.find((item) => item.id === id); if (procedure) next.set(id, selected.get(id) || { procedure, cost: defaultCost(procedure, workplaceId), duration: Number(procedure.duration) || 0 }); }); selected.clear(); next.forEach((value, id) => selected.set(id, value)); render(); } }); };
  m.querySelector('[data-record-add]').addEventListener('click', () => {}); render();
}
function openProcedureSettings({ procedure, current, onSave }) {
  ...
  const m = mountModal(document.body, modal(`<div class="record-screen record-screen--settings">${html}</div>`, { className: 'record-modal record-modal--panel' })); if (!m) return; initTimePickers(m); m.querySelector('[data-record-cancel]').addEventListener('click', () => m.remove()); m.querySelector('[data-record-save]').addEventListener('click', () => { const rawCost = String(m.querySelector('[data-record-cost]')?.value || '').replace(/[^0-9.,-]/g, '').replace(',', '.'); const timeValue = m.querySelector('[data-time-value]')?.value || '', match = timeValue.match(/^(\d{1,2}):(\d{2})$/); onSave?.({ procedure, cost: rawCost === '' ? '' : Number(rawCost), duration: match ? Number(match[1]) * 60 + Number(match[2]) : value.duration }); m.remove(); });
}
function openClientModal({ date, workplaceId, from, to, procedures: selectedProcedures, onCreated, onSelected }) {
  const all = people(); let filtered = all;
  const content = `<div class="record-screen record-screen--clients"><div class="record-client-toolbar"><input class="record-client-search" data-record-client-search placeholder="🔍 Найти клиента..." autocomplete="off"><button type="button" class="icon-button icon-button--primary" data-record-add-client aria-label="Добавить клиента">+</button></div><div class="record-client-list" data-record-client-list></div></div>`;
  const m = mountModal(document.body, modal(content, { className: 'record-modal record-modal--full' })); if (!m) return; let selectedClient = null;
  const render = () => { const host = m.querySelector('[data-record-client-list]'); host.innerHTML = list({ items: filtered.map((person) => ({ title: clientName(person), secondary: person.phones?.[0] || '', interactive: true, data: `data-record-client="${escapeHtml(person.key)}"`, selected: selectedClient?.key === person.key, aria: `Выбрать клиента ${clientName(person)}` })) }) || '<div class="muted">Клиенты не найдены.</div>'; host.querySelectorAll('[data-record-client]').forEach((row) => row.addEventListener('click', () => { selectedClient = all.find((person) => person.key === row.dataset.recordClient) || null; m.remove(); if (onSelected) { onSelected(selectedClient); return; } openClientConfirmation({ date, workplaceId, from, to, selectedClient, selectedProcedures, onCreated }); })); };
...
function workingDatesForWorkplace(workplaceId) { return getDays().filter((item) => String(item?.workplaceId || '') === String(workplaceId || '') && item?.date).map((item) => item.date).sort(); }
function openConfirmationWorkplaceModal({ workplaceId, onSelected }) {
  ...
  const m = mountModal(document.body, modal(content, { className: 'record-modal record-modal--full' })); if (!m) return;
  m.querySelectorAll('[data-record-workplace]').forEach((row) => row.addEventListener('click', () => { const selectedId = row.dataset.recordWorkplace || workplaceId; m.remove(); onSelected?.(selectedId); }));
}
function openConfirmationDateModal({ workplaceId, date, onSelected }) {
  ...
  const m = mountModal(document.body, modal(`<div class="record-screen record-screen--date"><div class="record-modal-toolbar"><strong>Выбор даты</strong></div><div data-record-confirm-calendar></div></div>`, { className: 'record-modal record-modal--full' })); if (!m) return;
  const calendarRoot = m.querySelector('[data-record-confirm-calendar]');
  initCalendar(calendarRoot, { month: new Date(current.getFullYear(), current.getMonth(), 1), workingDates, onDateSelect: (selectedDate) => { if (!workingDates.includes(selectedDate)) return; m.remove(); onSelected?.(selectedDate); } });
}
function availableConfirmationTimes({ date, workplaceId, duration }) {
  ...
  if (!workTime) return [];
  const start = timeToMinutes(workTime.from); const end = timeToMinutes(workTime.to); if (start == null || end == null) return [];
  const appointmentDuration = Math.max(1, Number(duration) || 0); const first = Math.ceil(start / 30) * 30; const result = [];
  for (let value = first; value + appointmentDuration <= end; value += 30) {
    const from = minutesToTime(value); const to = minutesToTime(value + appointmentDuration); if (isTimeRangeAvailable({ from, to, usages: scopedUsages(date, workplaceId) })) result.push({ from, to });
  }
  return result;
}
function openConfirmationTimeModal({ date, workplaceId, from, duration, onSelected }) {
  ...
  const m = mountModal(document.body, modal(content, { className: 'record-modal record-modal--full' })); if (!m) return;
  m.querySelectorAll('[data-record-confirm-time-option]').forEach((node) => node.addEventListener('click', () => { const selectedFrom = node.dataset.recordConfirmTimeOption || from; m.remove(); onSelected?.(selectedFrom); }));
}
