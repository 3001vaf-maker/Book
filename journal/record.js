import { entityCard, escapeHtml, listEntry, mountModal, modal, timePicker, initTimePickers, initMultiSelect } from '../ui/ui.js';
...
function openProceduresModal({ date, workplaceId, from, to, onCreated }) {
  ...
  const render = () => { const host = m.querySelector('[data-record-procedures]'); host.innerHTML = items.length ? items.map((procedure) => listEntry({ title: procedure.name || '', subtitle: durationText(procedure.duration), interactive: true, data: `data-procedure-select="${escapeHtml(procedure.id)}"`, aria: `Выбрать процедуру ${procedure.name || ''}`, actionData: `data-procedure-settings="${escapeHtml(procedure.id)}" data-selection-action`, actionAria: `Настройки процедуры ${procedure.name || ''}`, actionIcon: '⚙', className: selected.has(procedure.id) ? 'is-selected' : '' })).join('') : '<div class="muted">Процедур для этого места работы пока нет.</div>'; syncActions(); selectionController?.destroy(); selectionController = initMultiSelect(host, { selectedValues: [...selected.keys()], selector: '[data-procedure-select]', valueAttribute: 'procedureSelect', onChange: (values) => { const next = new Map(); values.forEach((id) => { const procedure = items.find((item) => item.id === id); if (procedure) next.set(id, selected.get(id) || { procedure, cost: defaultCost(procedure, workplaceId), duration: Number(procedure.duration) || 0 }); }); selected.clear(); next.forEach((value, id) => selected.set(id, value)); syncActions(); } }); m.querySelectorAll('[data-procedure-settings]').forEach((node) => node.addEventListener('click', (event) => { event.preventDefault(); event.stopPropagation(); openProcedureSettings(m, items.find((item) => item.id === node.dataset.procedureSettings), selected, selectionController, workplaceId); })); };
  m.querySelector('[data-record-add]').addEventListener('click', () => {}); render();
}
function openProcedureSettings(parent, procedure, selected, selectionController, workplaceId) {
  if (!procedure) return; const current = selected.get(procedure.id) || { procedure, cost: defaultCost(procedure, workplaceId), duration: Number(procedure.duration) || 0 };
