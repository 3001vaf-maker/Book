...
assert.match(v2Ui, /const relation = index - activeIndex/);
assert.match(v2Ui, /const depthX = visualDepth \* 16/);
assert.match(v2Ui, /const depthY = visualDepth \* 16/);
assert.match(v2Ui, /\.slice\(0, 7\)/);
assert.match(v2Ui, /data-v2-f-level="F\$\{index \+ 1\}"/);
assert.match(v2Ui, /const nextIndex = finalDx < 0 \? activeIndex \+ 1 : activeIndex - 1/);
assert.doesNotMatch(v2Ui, /\(activeIndex \+ direction \+ cards\.length\) % cards\.length/);
...
assert.match(v2Css, /border:1px solid rgba\(17,17,17,.28\)/);
