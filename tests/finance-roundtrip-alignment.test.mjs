...
assert.match(paymentUi, /settlement\?\.planTotal[\s\S]*state\?\.paidTotal/);
const correctionStart = paymentUi.indexOf('async function saveSettlementCorrection');
const correctionSource = paymentUi.slice(correctionStart, paymentUi.indexOf('function paymentEntryContent', correctionStart));
assert.ok(correctionSource.indexOf('await saveSettlementSnapshot') >= 0);
assert.ok(correctionSource.indexOf('const updated = updateRecord') > correctionSource.indexOf('await saveSettlementSnapshot'));
