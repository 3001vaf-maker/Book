...
const service = await readFile(new URL('../server/src/online-booking/person-card-link.service.ts', import.meta.url), 'utf8');
...
assert.doesNotMatch(service, /await this\.reconcileLegacyAccountDuplicates\(tenantId\)/, 'Person login/access must not run identity reconciliation automatically');

const cardState = service.slice(service.indexOf('async cardState('), service.indexOf('private assertUnambiguousPhone('));
assert.match(cardState, /personHasPhone\(person, phone\)/, 'Person profile lookup must use phone');
assert.doesNotMatch(cardState, /email/i, 'Email must never choose an existing person profile');
assert.doesNotMatch(cardState, /telegram/i, 'Telegram must never choose an existing person profile');
...
assert.match(manualRecords, /phonesMatch\(person\.phone, account\.phone\)/, 'Master-created records must remain discoverable by the registered contact phone');
...
assert.match(myRequests, /return \[\.\.\.requestViews, \.\.\.manualViews\]/, 'Online and pre-existing master records must be returned in one person history');

assert.match(controller, /owner\/reconcile-legacy-person-cards/, 'Owner-only legacy review route is retained for compatibility');
assert.match(controller, /@UseGuards\(JwtAuthGuard\)[\s\S]*reconcileLegacyPersonCards/, 'Legacy review route must require owner authentication');
assert.doesNotMatch(sync, /legacyPersonCardsReconciled/, 'Book startup must not keep automatic legacy reconciliation state');
assert.doesNotMatch(sync, /\/online-booking\/owner\/reconcile-legacy-person-cards/, 'Book startup must never trigger legacy Person/UEI reconciliation');

console.log('legacy-person-card-reconciliation.test.mjs: ok');
