...
const workspaceCascadeMigration = fs.readFileSync('server/prisma/migrations/20260922082000_workspace_state_profile_cascade/migration.sql', 'utf8');
...
assert.doesNotMatch(workspaceCascadeMigration, /DROP TABLE IF EXISTS/);
assert.doesNotMatch(workspaceCascadeMigration, /DROP FUNCTION IF EXISTS/);
assert.match(workspaceCascadeMigration, /WorkspaceState_tenantId_fkey/);
assert.match(workspaceCascadeMigration, /WorkspaceState_platformAccountId_fkey/);
assert.match(workspaceCascadeMigration, /ON DELETE CASCADE ON UPDATE CASCADE/);
