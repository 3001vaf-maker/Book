...
assert.match(firstRun, /Пользователь ещё не запросил переход в LIVE/);
...
assert.match(runtime, /Вы можете запросить LIVE уже сейчас/);
assert.doesNotMatch(firstRun, /commercialMode !== 'LIVE' \|\| progress/);
assert.match(firstRun, /cleanupDemoOperationalData/);
assert.match(firstRun, /access\.commercialMode === 'LIVE' && \(access\.isOwnerBook \|\| access\.liveApprovedAt\)/);
...
assert.match(admin, /Подтвердить LIVE/);
assert.match(admin, /Запрос LIVE от пользователя ещё не поступал/);
