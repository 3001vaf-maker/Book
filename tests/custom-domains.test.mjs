...
const masterInvitation = readFileSync(new URL('../server/src/master-invitation/master-invitation.service.ts', import.meta.url), 'utf8');
...
assert.match(masterInvitation, /https:\/\/book\.va-tools\.ru/);
assert.match(bookingSettings, /CLIENT_APP_ORIGIN/);
assert.doesNotMatch(bookingSettings, /window\.location\.origin/);
assert.match(core, /redirectBookingToClient/);
