  ...
  app.innerHTML = `
    <main class="admin-login">
      <section class="admin-login-card">
        <h1>Book Admin</h1>
        <p>Управление персональными Book профилей</p>
    ...
    <div class="admin-shell">
      <aside class="admin-sidebar">
        ...
        <nav class="admin-nav">
          ...
          <button data-section="profiles">Профили</button>
...
function renderOverview() {
  ...
  const regular = state.profiles.filter((item) => !item.isOwnerBook);
  const active = regular.filter((item) => item.status === 'ACTIVE' && item.profile).length;
  const pending = regular.filter((item) => !item.profile && item.invitation?.status === 'PENDING').length;
  content.innerHTML = `
    <div class="admin-heading"><div><h2>Обзор</h2><p>Состояние персональных Book</p></div></div>
    <div class="admin-stats">
      <div class="admin-stat"><strong>${regular.length}</strong><span>создано Book профилей</span></div>
...
function renderOwnerBook() {
  ...
  const owner = state.profiles.find((item) => item.isOwnerBook);
  const content = app.querySelector('[data-content]');
  if (!owner) {
  ...
  content.innerHTML = `
    ...
    <div class="admin-card" style="padding:20px">
      <strong>${escapeHtml(owner.profile?.name || owner.tenantName)}</strong>
      <p style="color:#817a74">${escapeHtml(owner.profile?.email || '')}</p>
...
function renderProfiles() {
  ...
  const content = app.querySelector('[data-content]');
  const profiles = state.profiles.filter((item) => !item.isOwnerBook);
  content.innerHTML = `
    ...
    <div class="admin-card">
      <table class="admin-table">
        ...
        <tbody>${profiles.map(profileRow).join('') || '<tr><td colspan="4">Пока нет приглашённых профилей.</td></tr>'}</tbody>
      </table>
    </div>`;
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      await refreshData();
      window.setTimeout(renderProfiles, 350);
    } catch (error) {
...
function profileRow(item) {
  const pending = !item.profile && item.invitation?.status === 'PENDING';
  const name = item.profile?.name || item.invitation?.name || item.tenantName;
  const email = item.profile?.email || item.invitation?.email || '';
  const statusClass = item.status === 'SUSPENDED' ? 'suspended' : pending ? 'pending' : 'active';
  const statusLabel = item.status === 'SUSPENDED' ? 'Отключён' : pending ? 'Ждёт входа' : item.profile ? 'Активен' : 'Создан';
...
function openAccessDrawer(tenantId) {
  const profile = state.profiles.find((item) => item.tenantId === tenantId);
  if (!profile) return;
  const resolved = new Map((profile.access?.capabilities || []).map((item) => [item.key, item]));
  const backdrop = document.createElement('div');
  backdrop.className = 'admin-drawer-backdrop';
  backdrop.innerHTML = `
    <aside class="admin-drawer">
      <div class="admin-drawer-head">
        <div><h3>${escapeHtml(profile.isOwnerBook ? 'Мой Book' : (profile.profile?.name || profile.invitation?.name || profile.tenantName))}</h3><p>${escapeHtml(profile.profile?.email || profile.invitation?.email || '')}</p></div>
      ...
      <section class="admin-section">
        <h4>Состояние</h4>
        <button class="admin-button ${profile.status === 'SUSPENDED' ? '' : 'danger'}" data-status>${profile.status === 'SUSPENDED' ? 'Включить Book' : 'Отключить Book'}</button>
      </section>
