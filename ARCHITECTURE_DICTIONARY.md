...
- один Person может быть связан с одним или несколькими Account; у одного человека может быть несколько разных точек входа, например несколько телефонов, email или Telegram;
- несколько разных Account могут относиться к одной и той же Person;
- в пределах одного Tenant одинаковый нормализованный телефон всегда означает того же Person: при совпадении телефона существующий Person переиспользуется, новый Person не создаётся;
- принадлежность Account к Person и доступ Account к другой Person — разные отношения; доступ не объединяет личности и не меняет UEI;
- совпадение имени, email или другого отдельного признака без совпадения телефона само по себе не является достаточным основанием создавать identity-связь;
- UEI связывает Person-записи, которые явно признаны одной идентичностью, и не должен использоваться для создания второго Person при уже известном телефоне;
- связывание и разъединение Person через UEI не должно терять Record, Finance, Communication, Documents/Consents или другую историю.
