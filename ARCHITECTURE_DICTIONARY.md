...
- один Person может иметь один или несколько Account; несколько разных Account могут принадлежать одной Person;
- каждый Account в identity-контуре принадлежит одной Person; право этого Account видеть/обслуживать другую Person является отдельным access-отношением и не меняет identity;
- один и тот же нормализованный входящий контакт распознаёт тот же Account и ту же Person; новый Account сам по себе не является основанием создавать новую Person;
- исторические дубли Person нельзя автоматически объединять по имени/телефону с изменением UEI: cleanup должен сохранять Record, Finance, Communication, Documents/Consents и другую историю;
- UEI остаётся отдельным identity-якорем и не смешивается с Account, Contact или Profile.
