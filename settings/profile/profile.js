import { accordion, actionBlock, button, collectRepeatedField, entityCard, field, iconButton, initAccordions, initPhotoField, initRepeatedFields, modal, mountModal, page, photoField, repeatedField, select, textareaField, workplaceCountText } from '../../ui/ui.js';
...
function renderProfile(root,navigateBack){
  ...
  const workplaces=`<section class="workplaces-section"><div class="section-heading"><h2>Рабочие места</h2>${iconButton('+',{className:'icon-button--primary',data:'data-add-workplace',aria:'Добавить рабочее место'})}</div>${workplaceList()}</section>`;
  root.innerHTML=page([
    profileCard(p),
    accordion(items),
    workplaces,
