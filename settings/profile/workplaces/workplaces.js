import { actionBlock, button, collectLinks, emptyState, entityCard, escapeHtml, initLinks, initPhotoField, initTimePickers, links, listEntries, listEntry, mountModal, modal, page, phoneField, photoField, searchableSelect, select, textareaField, timeInput } from '../../../ui/ui.js';
...
export function renderWorkplace(root,key,navigateBack=()=>{}){
  ...
  const details=`<div class="workplace-details"><div><span>Город</span><strong>${escapeHtml(w.city||'—')}</strong></div><div><span>Адрес</span><strong>${escapeHtml(w.address||'—')}</strong></div><div><span>Валюта</span><strong>${escapeHtml(w.currency||'—')}</strong></div>${w.about?`<div><span>О рабочем пространстве</span><strong>${escapeHtml(w.about)}</strong></div>`:''}${w.links?.length?`<div><span>Рабочие ссылки</span><strong>${escapeHtml(w.links.map(l=>`${l.type}: ${l.url}`).join(', '))}</strong></div>`:''}</div>`;
  root.innerHTML=page([card,details,actionBlock(`${button('Изменить',{data:'data-edit-workplace'})}${button('Назад',{className:'ui-button--secondary',data:'data-back-workplaces'})}`)]);
