...
export function render(root, navigateBack = () => {}) {
  root.innerHTML = `${pageHeader('Подсказки по сообщениям')}
    <div class="action-block"><strong>Структура</strong><div class="muted">Приветствие → причина сообщения → конкретное действие → контакт отправителя/бизнеса при необходимости.</div></div>
