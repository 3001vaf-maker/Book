...
function renderList(root) {
  const items = list();
  root.innerHTML = `<div class="entity-page-header">${pageHeader('Ярлыки')}<div class="page-header-action">${iconButton('+', { className: 'icon-button--primary', data: 'data-add-tag', aria: 'Добавить ярлык' })}</div></div>${items.length ? `<div class="tag-entity-list">${items.map(renderRow).join('')}</div>` : emptyState('Ярлыков пока нет', 'Добавьте первый ярлык кнопкой «+».')}`;
  root.querySelector('[data-add-tag]')?.addEventListener('click', () => openForm(root));
  root.querySelectorAll('[data-delete-tag]').forEach((element) => element.addEventListener('click', (event) => {
    event.stopPropagation();
    deleteTag(root, element.dataset.deleteTag);
  }));
}
...
function saveTag(root, modalRoot) {
